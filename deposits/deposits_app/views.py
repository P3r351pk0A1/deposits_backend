...
class MiningServiceMethods(APIView):
    ...
    @swagger_auto_schema(query_serializer = MServicesListSerializer, responses = {200:MiningServicesListResponseSerializer})
    def get (self, request, format = None):
        ...
        try:
            MServicesInCurOrder = LinkServiceOrderSerializer(LinkServicesOrders.objects.filter(mining_order_id = UsersDraftId), many = True).data
        except Exception:
            MServicesInCurOrder = []
        ...
        return Response({
            ...
            'MServicesInCurOrder': MServicesInCurOrder
        })  
...
class MiningServiceMethods_byId(APIView):
    ...
    def post(self, request, pk, format = None):
        ...
        if not LinkServicesOrders.objects.filter(mining_order_id = draftOrder.mining_order_id, mining_service_id = Mining_Service.mining_service_id).exists():
            ...
            return Response({                
                'MiningServicesInUsersDraft': LinkServicesOrders.objects.filter(mining_order_id=draftOrder.mining_order_id).count() ,
                'UsersDraftId': draftOrder.pk})
...
class MiningOrdersMethods(APIView):
    ...
    @swagger_auto_schema(responses = {200:MiningOrdersListResponseSerializer})
    @method_permission_classes([IsAuth])        
    def get(self, request, format = None):
        CurUser = getUserBySession(request)
...
class MiningOrderMethods_byId(APIView):
    ...
    @swagger_auto_schema(responses = {200:SingleMiningOrderSerializer})
    @method_permission_classes([IsAuth])
    def get(self, request, pk, format = None):
        CurUser = getUserBySession(request)
        if not (CurUser.is_staff or CurUser.is_superuser):
    ...
    @swagger_auto_schema(request_body = MiningOrdermmfieldsSerializer)   
    @method_permission_classes([IsAuth])
    def put(self, request, pk, format = None):
        CurUser = getUserBySession(request)
        if not (CurUser.is_staff or CurUser.is_superuser):
    ...
    def delete(self, request, pk, format = None):
        ...
        if Mining_Order.status != 'draft':
            return Response({'error':'This order is saved and cant be deleted'}, status=status.HTTP_400_BAD_REQUEST)
...
class LinkMiningServisesOrdersMethods(APIView):
    ...
    @swagger_auto_schema(responses = {200:MiningServiceOrderSerializer})
    @method_permission_classes([IsAuth])
    def delete(self, request, pk_mservice, pk_morder, format = None):
        ...
        MiningServise_link = get_object_or_404(self.model_class, mining_order_id = Mining_Order.pk, mining_service_id = pk_mservice)
    ...
    def put(self, request, pk_mservice, pk_morder, format = None):
        ...
        serializer = self.serializer_class(MiningServise_link, data = request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_202_ACCEPTED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class UserRegistration(APIView):
    ...
    def post(self, request, format = None):
        time.sleep(1)
        CurUser = getUserBySession(request)
        if request.data['username'] == '' or request.data['password'] == '' or request.data['email'] == '' or request.data['first_name'] == '' or request.data['last_name'] == '':
            return Response({'status': 'Error. Some fields are empty'}, status=status.HTTP_400_BAD_REQUEST)
        if self.model_class.objects.filter(username=request.data['username']).exists() or self.model_class.objects.filter(email=request.data['email']).exists():
            return Response({'status': 'Error. Username whith this name or email exists', }, status=status.HTTP_400_BAD_REQUEST )
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():           
    ...
    def put(self, request): 
        ...
        if serializer.is_valid():
            serializer.save()                               
            if 'password' in serializer.validated_data and serializer.validated_data.get('password'):
...
class Authentication(APIView):
    ...
    def post(self, request, format = None):
        ...
        if cur_user != None:
            ...
            resp =  Response(self.serializer_class(cur_user).data, status=status.HTTP_200_OK                            )    
